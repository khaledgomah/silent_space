...
class AmbientBuilderWidget extends StatelessWidget {
  const AmbientBuilderWidget({super.key});

  static List<Sound> _buildSounds(BuildContext context) => [
        Sound(
  ...
  Widget build(BuildContext context) {
    final sounds = _buildSounds(context);

    return BlocBuilder<TimerCubit, TimerState>(
      builder: (context, state) {
        final selectedIndex = sounds.indexWhere((sound) => sound.path == state.path);
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) => FittedBox(
            child: SoundTile(
              ...
              onPressed: () {
                context.read<TimerCubit>().setPath(sounds[index].path);
              },
              sound: sounds[index],
            ),
          ),
